  ...
  class ResourceProperty{
    ...
    var columnType:ColumnType = ColumnType.String
      ...
      def createIndexField(value: Any): Field= {
          columnType.getColumnType.asInstanceOf[MonadColumnType[Any]].createIndexField(value,this)
      }
      def setIndexValue(f:Field,value:Any){
          columnType.getColumnType.asInstanceOf[MonadColumnType[Any]].setIndexValue(f,value,this)
      }
      def isToken: Boolean = {
          indexType.indexType() == Field.Index.ANALYZED
      }
      def isKeyword: Boolean = {
          indexType.indexType() == Field.Index.NOT_ANALYZED
      }
      def isNumeric:Boolean={
          columnType == ColumnType.Long || columnType == ColumnType.Int || columnType == ColumnType.Date
      }
      def readJdbcValue(rs:ResultSet,index:Int)={
          columnType.getColumnType.readValueFromJdbc(rs,index,this)
      }
      def readDfsValue(dbObj:JsonObject)={
          try{
              columnType.getColumnType.readValueFromDfs(dbObj,this)
          }catch{
              case e:Throwable =>
                throw new MonadException("unable to read value from dfs with name:"+this.name,e,null)
          }
      }
      def readApiValue(dbObj:JsonObject)={
          columnType.getColumnType.asInstanceOf[MonadColumnType[Any]].convertDfsValueToString(readDfsValue(dbObj),this)
      }
  }
